...
**has_invoice** | **bool** | Set to true to create a draft invoice (adds an auto incrementing invoice number if none is provided) even if there are no line_items or tax passed | [optional] 
...
payment_request_create_form_dict = payment_request_create.from_dict(payment_request_create_dict)
```
